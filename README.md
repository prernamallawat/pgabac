...
The simple ABAC workload has additional overhead because PostgreSQL evaluates an RLS policy and calls `abac_check_access(...)`.
...
The current prototype prioritizes correctness and flexibility over performance. The large overhead is mainly due to dynamic metadata lookup and per-row PL/pgSQL rule evaluation inside RLS.
